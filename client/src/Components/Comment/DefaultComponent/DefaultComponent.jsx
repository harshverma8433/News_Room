...
console.log(user);
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const uri = `${process.env.REACT_APP_API_URL}/getcomments/${articleId}`;
  ...
  const handleLikeComment = async (commentId) => {
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/likecomment/${commentId}`, {}, { 
        headers : {
          Authorization : `${token}`
        }
       });
      ...
      if (response.status === 200) {
        ...
        setData(prevData => prevData.map(comment =>
          comment.comId === commentId ? { ...comment, likes: comment.likes + 1, likedByUser: true } : comment
        ));
      }
    } catch (error) {
  ...
  const handleUnlikeComment = async (commentId) => {
    try {
      const token = localStorage.getItem("jwt");
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/unlikecomment/${commentId}`, {}, { 
        headers : {
      ...
      if (response.status === 200) {
        ...
        setData(prevData => prevData.map(comment =>
          comment.comId === commentId ? { ...comment, likes: comment.likes - 1, likedByUser: false } : comment
        ));
      }
    } catch (error) {
  ...
  const handleCommentSubmit = async (newComment) => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/addcomment`, {
  ...
  const handleReplySubmit = async (commentId, replyText) => {
    if (!replyText || replyText.trim() === "") {
      alert("Reply text is empty or undefined");
      return;
    }

    try {
      const token = localStorage.getItem("jwt");
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/replycomment/${commentId}`, {
        text: replyText,
      }, { 
        headers : {
          'Authorization' : ` ${token}`
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      const response = await axios.delete(
        `${process.env.REACT_APP_API_URL}/deletecomment/${commentId}`,
        { withCredentials: true }
      );
  
      if (response.status === 200) {
        // Remove the comment from the state
        setData(prevData => prevData.filter(comment => comment.comId !== commentId));
      } else {
  ...
  const handleEditComment = async (commentId, newText) => {
    try {
      const response = await axios.put(
        `${process.env.REACT_APP_API_URL}/updatecomment/${commentId}`,
        { text: newText },
        { withCredentials: true }
      );
      ...
      if (response.status === 200) {
        ...
        setData(prevData =>
          prevData.map(comment =>
            comment.comId === commentId ? { ...comment, text: newText } : comment
          )
        );
      } else {
    ...
    <CommentSection
      ...
      onReplyAction={(replyData) => {
        ...
        if (!repliedToCommentId || !text) {
          alert("Reply text or comment ID is undefined or null. Please ensure you entered text.");
          return;
        }
      ...
      renderLikeButton={(comment) => (
        <span onClick={() => comment.likedByUser ? handleUnlikeComment(comment.comId) : handleLikeComment(comment.comId)}>
          {comment.likedByUser ? <AiFillHeart color="red" /> : <AiOutlineHeart />}
        </span>
      )}
      onDeleteAction={(commentId) => handleDeleteComment(commentId)}
      onEditAction={(commentId, newText) => handleEditComment(commentId, newText)}
      currentData={(data) => {
