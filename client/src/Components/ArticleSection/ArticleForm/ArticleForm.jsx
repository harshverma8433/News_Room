...
const ArticleForm = () => {
  ...
  const handleNewsPayload = (event) => {
    ...
    setNewsPayload((prevNewsPayload) => ({ ...prevNewsPayload, [name]: value }));
  };
  ...
  return (
    ...
    >
      <Toaster />
      <h1 className="text-6xl tracking-wider text-white py-6 font-serif">Add Article</h1>
      ...
      <div className="flex items-center justify-center flex-col border-2 w-[550px] border-slate-700 border-dotted px-5 py-2 rounded-xl">
        <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full rounded-lg cursor-pointer">
          ...
          <input name="image" id="dropzone-file" type="file" className="hidden" onChange={handleFile} />
        </label>
        {file && <img src={URL.createObjectURL(file)} className="w-96 h-60 mt-4 rounded-md" alt="Preview" />}
      </div>
