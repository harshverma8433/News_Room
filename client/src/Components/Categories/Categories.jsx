...
const Categories = ({ active, handleCategory }) => {
  const categoryItems = [
    ...
    { id: 4, name: "Business", icon: <PiBuildingOfficeFill />, navigateLink: "/" },
    { id: 5, name: "Social Media", icon: <TiSocialYoutube />, navigateLink: "/" },
    { id: 6, name: "Entertainment", icon: <MdLocalMovies />, navigateLink: "/" },
  ...
  const isActive = (categoryName) => (active === categoryName ? 'bg-gray-600' : '');

  return (
    <div className="pb-6 bg-slate-900">
      <hr className="border border-slate-800" />
      <h1 className="text-center py-4 text-2xl tracking-wider font-serif">Categories</h1>
      ...
      <div className="flex flex-col justify-center items-center gap-y-5 mt-6">
        {categoryItems.map((item) => (
          <div
            ...
            className={`flex rounded-xl items-center pl-8 gap-x-3 w-52 h-12 hover:bg-slate-400 cursor-pointer ${isActive(item.name)}`}
          >
